...
Description   :Script to create a ram graph
...
with open('../../data/ram.dat', 'r') as csvfile:
    ...
    for row in data_source:
        ...
        free_mem.append(float((int(row[1])/1024)+(int(row[4])/1024)+(int(row[5])/1024)))
        used_mem.append(float((int(row[2])/1024)-(int(row[4])/1024)-(int(row[5])/1024)))
        buffer_mem.append(float(int(row[4])/1024))
        cached_mem.append(float(int(row[5])/1024))
