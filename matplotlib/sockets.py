...
Description   :Script to create sockets graph
...
with open('../../data/sockets.dat', 'r') as csvfile:
    ...
    for row in data_source:
        ...
        t_tcp.append(float((int(row[2]))+(int(row[6]))))
        t_tcp_use.append(float(row[2]))
        t_udp_use.append(float(row[3]))
        t_tcp_time_wait.append(float(row[6]))
